...
func (a *Client) SolSendTransaction(params *SolSendTransactionParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*SolSendTransactionOK, error) {
	...
	op := &runtime.ClientOperation{
		...
		Method:             "POST",
		PathPattern:        "/public/v1/submit/sol_send_transaction",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &SolSendTransactionReader{formats: a.formats},
		AuthInfo:           authInfo,
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	...
	success, ok := result.(*SolSendTransactionOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for SolSendTransaction: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
