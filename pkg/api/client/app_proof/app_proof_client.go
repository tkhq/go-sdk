...
func (a *Client) GetAppProofs(params *GetAppProofsParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*GetAppProofsOK, error) {
	...
	op := &runtime.ClientOperation{
		ID:                 "GetAppProofs",
		Method:             "POST",
		PathPattern:        "/public/v1/query/list_app_proofs",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &GetAppProofsReader{formats: a.formats},
		AuthInfo:           authInfo,
	...
	success, ok := result.(*GetAppProofsOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for GetAppProofs: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
