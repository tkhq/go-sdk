...
func main() {
	...
	walletID := mustEnv("TURNKEY_WALLET_ID")
	...
	result, err := client.ExportWallet(ctx, turnkey.ExportWalletRequest{
		WalletID:        walletID,
		TargetPublicKey: targetPublicKey,
	})
	if err != nil {
		fatalRequestError(err, "export wallet")
	}

	mnemonic, err := crypto.DecryptExportBundle([]byte(result.ExportBundle), organizationID, kemPrivateKey)
	if err != nil {
